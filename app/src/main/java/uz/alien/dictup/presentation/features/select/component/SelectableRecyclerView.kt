...
class SelectableRecyclerView @JvmOverloads constructor(
    ...
    private fun updateScrollDistance() {
        ...
        if (child != null) {
            ...
            var itemHeight = lm?.getDecoratedMeasuredHeight(child) ?: 0

            if (itemHeight <= 0) {
                val lp = child.layoutParams as? MarginLayoutParams
                val margins = (lp?.topMargin ?: 0) + (lp?.bottomMargin ?: 0)
                itemHeight = (child.measuredHeight.takeIf { it > 0 } ?: child.height) + margins
            }
