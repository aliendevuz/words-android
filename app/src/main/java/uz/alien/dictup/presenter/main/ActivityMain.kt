...
class ActivityMain : ActivityBase() {
    ...
    override fun onReady(savedInstanceState: Bundle?) {
        ...
        if (PrefsManager.Companion.getInstance(this)
                .getBool(PrefsManager.Companion.IS_FIRST_TIME, true)
        ) {
            PrefsManager.Companion.getInstance(this)
                .saveBool(PrefsManager.Companion.IS_FIRST_TIME, false)
    ...
    fun setHomeScreen() {
        ...
        mainBinding.root.startAnimation(AnimationSet(false).apply {
            ...
            duration = this@ActivityMain.duration * 3
        })

        val filter = IntentFilter(ConnectivityManager.CONNECTIVITY_ACTION)
        registerReceiver(ConnectionReceiver {
            downloadAndSaveFiles(this, "beginner_en", PrefsManager.Companion.BEGINNER_EN) {
                Log.d("Network", "Updated!")
                runOnUiThread {
                    Toast.makeText(this, "Updated!", Toast.LENGTH_SHORT).show()
                }
            }
            downloadAndSaveFiles(this, "beginner_uz", PrefsManager.Companion.BEGINNER_UZ) {
                Log.d("Network", "Updated!")
                runOnUiThread {
                    Toast.makeText(this, "Updated!", Toast.LENGTH_SHORT).show()
                }
            }
            downloadAndSaveFiles(this, "beginner_story", PrefsManager.Companion.BEGINNER_STORY) {
                Log.d("Network", "Updated!")
                runOnUiThread {
                    Toast.makeText(this, "Updated!", Toast.LENGTH_SHORT).show()
                }
            ...
            downloadAndSaveFiles(this, "essential_en", PrefsManager.Companion.ESSENTIAL_EN) {
                Log.d("Network", "Updated!")
                runOnUiThread {
                    Toast.makeText(this, "Updated!", Toast.LENGTH_SHORT).show()
                }
            }
            downloadAndSaveFiles(this, "essential_uz", PrefsManager.Companion.ESSENTIAL_UZ) {
                Log.d("Network", "Updated!")
                runOnUiThread {
                    Toast.makeText(this, "Updated!", Toast.LENGTH_SHORT).show()
                }
            }
            downloadAndSaveFiles(this, "essential_story", PrefsManager.Companion.ESSENTIAL_STORY) {
                Log.d("Network", "Updated!")
                runOnUiThread {
                    Toast.makeText(this, "Updated!", Toast.LENGTH_SHORT).show()
                }
    ...
    companion object {
        ...
        private fun sendTokenToServer(token: String) {
            ...
            val body = RequestBody.Companion.create("application/json".toMediaTypeOrNull(), json)
        ...
        fun downloadAndSaveFiles(
            ...
            fetchFile(fileName) {
                it?.let { v ->
                    if (v.toInt() > PrefsManager.Companion.getInstance(context).getInt(version)) {
                        fetchFile("$fileName.json") {
                            it?.let { content ->
                                val dir = File(context.filesDir, "src")
                                if (!dir.exists()) dir.mkdirs()
                                File(dir, "$fileName.json").writeText(content)
                                PrefsManager.Companion.getInstance(context)
                                    .saveInt(version, v.toInt())
                                runnable?.run()
                            }
                        }
                    }
        ...
        fun showInitialDialog(context: Context) {
            AlertDialog.Builder(context)
                ...
                .setPositiveButton("OK") { _, _ ->
                    if (ConnectionReceiver.Companion.isConnected(context)) {
                        val success = downloadAndSaveFiles(
                            context, "beginner_en",
                            PrefsManager.Companion.BEGINNER_EN
                                ...
                                downloadAndSaveFiles(
                                    context, "beginner_story",
                                    PrefsManager.Companion.BEGINNER_STORY
                                ) &&
                                downloadAndSaveFiles(
                                    context, "essential_en",
                                    PrefsManager.Companion.ESSENTIAL_EN
                                ) &&
                                downloadAndSaveFiles(
                                    context, "essential_uz",
                                    PrefsManager.Companion.ESSENTIAL_UZ
                                ) &&
                                downloadAndSaveFiles(
                                    context, "essential_story",
                                            ...
                                            handler.postDelayed(1000L) {
                                                ...
                                                handler.postDelayed(2000L) {
                                                    ...
                                                    it.root.startAnimation(AnimationSet(false).apply {
                                                        ...
                                                        duration = this@ActivityMain.duration / 2
                                                    })
